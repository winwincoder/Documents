 ...
 */
static ALWAYS_INLINE float qp2qscale( float qp )
{
    return 0.85f * powf( 2.0f, ( qp - 12.0f ) / 6.0f );
}
static ALWAYS_INLINE float qscale2qp( float qscale )
{
...
static ALWAYS_INLINE double qscale2bits( ratecontrol_entry_t *rce, double qscale )
{
    if( qscale<0.1 )
        qscale = 0.1;
...
void x264_adaptive_quant_frame( x264_t *h, x264_frame_t *frame, float *quant_offsets )
{
    ...
    {
        uint32_t energy;
        float bit_depth_correction, avg_adj_pow2, qp_adj ;
		
        if( h->param.rc.i_aq_mode == X264_AQ_AUTOVARIANCE )
        {
            bit_depth_correction = powf(1 << (BIT_DEPTH-8), 0.5f);
            avg_adj_pow2 = 0.f;
            for( mb_y = 0; mb_y < h->mb.i_mb_height; mb_y++ )
                for( mb_x = 0; mb_x < h->mb.i_mb_width; mb_x++ )
                {
                    energy = x264_ac_energy_mb( h, mb_x, mb_y, frame );
                    qp_adj = powf( energy + 1, 0.125f );
        ...
        for( mb_y = 0; mb_y < h->mb.i_mb_height; mb_y++ )
            ...
            {
                //float qp_adj;
                mb_xy = mb_x + mb_y*h->mb.i_mb_stride;
                if( h->param.rc.i_aq_mode == X264_AQ_AUTOVARIANCE )
                {
...
static int x264_macroblock_tree_rescale_init( x264_t *h, x264_ratecontrol_t *rc )
{
    ...
    srcdim[0] = rc->mbtree.srcdim[0] / 16.f;
    srcdim[1] = rc->mbtree.srcdim[1] / 16.f;
    dstdim[0] = h->param.i_width / 16.f;
    dstdim[1] = h->param.i_height / 16.f;
    srcdimi[0] = ceil(srcdim[0]);
    srcdimi[1] = ceil(srcdim[1]);
    dstdimi[0] = ceil(dstdim[0]);
    dstdimi[1] = ceil(dstdim[1]);
...
void x264_ratecontrol_init_reconfigurable( x264_t *h, int b_init )
{
    ...
    {
         int kilobit_size, vbv_buffer_size, vbv_max_bitrate;
...
int x264_ratecontrol_mb( x264_t *h, int bits )
{
    ...
    float prev_row_qp,qp_absolute_max, qp_max,qp_min,step_size,buffer_left_planned,slice_size_planned, max_frame_error, size_of_other_slices;
    ...
    {
        float size_of_other_slices_planned = 0, weight;
	 int i;
...
static void update_predictor( predictor_t *p, float q, float var, float bits )
{
    float range = 1.5, old_coeff, new_coeff, new_coeff_clipped, new_offset;
    if( var < 10 )
        return;
...
static double clip_qscale( x264_t *h, int pict_type, double q )
{
    ...
    if( rcc->b_vbv && rcc->last_satd > 0 )
    {
        ...
        {
            double bits, max_fill_factor, min_fill_factor;
            if( ( pict_type == SLICE_TYPE_P ||
...
static int init_pass2( x264_t *h )
{
    ...
    double rate_factor, step_mult, qblur, cplxblur, expected_bits, base_cplx;
