...
static NOINLINE unsigned int x264_weight_cost_luma( x264_t *h, x264_frame_t *fenc, pixel *src, x264_weight_t *w )
{
    ...
    if( w )
    {
        ...
        for( y = 0; y < i_lines; y += 8, pixoff = y*i_stride )
            for( x = 0; x < i_width; x += 8, i_mb++, pixoff += 8 )
            {
                 cmp = h->pixf.mbcmp[PIXEL_8x8]( &src[pixoff], i_stride, &fenc_plane[pixoff], i_stride );
                cost += X264_MIN( cmp, fenc->i_intra_cost[i_mb] );
            }
    x264_emms();
    return cost;
	  ...
	  pixel *mcbuf;
		...
		
        {
		  ...
		  int i_scale;
	        ...
	        {
	            int start_offset, end_offset, i_off;
                                    ...
                                    int *output_inter, int *output_intra )
{
    ...
    fenc_mvs[0] = &fenc->lowres_mvs[0][b-p0-1][i_mb_xy];
    fenc_mvs[1] = &fenc->lowres_mvs[1][p1-b-1][i_mb_xy] ;
    fenc_costs[0] = &fenc->lowres_mv_costs[0][b-p0-1][i_mb_xy];
    fenc_costs[1] = &fenc->lowres_mv_costs[1][p1-b-1][i_mb_xy];
                                      ...
                                      int b_intra_penalty )
{
    ...
    else
    {
	 ...
	 int output_buf_size;
        ...
        {
             int i;
	      int *row_satd_inter, *row_satd_intra;
			...
			s[i].p1 = p1;
		       s[i].b = b;
			s[i].dist_scale_factor = dist_scale_factor;
		       s[i].do_search = do_search;
			s[i].w = w;
			s[i].output_inter = output_inter[i];
		       s[i].output_intra = output_intra[i];
		   ...
		   //s = (x264_slicetype_slice_t){ h, a, frames, p0, p1, b, dist_scale_factor, do_search, w,
		  ...
		  s.w = w;
		  s.output_inter = output_inter[0];
		  s.output_intra = output_intra[0];
...
static void x264_macroblock_tree( x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int num_frames, int b_intra )
{
    ...
    int bframes = 0;
    float total_duration, average_duration;
    int j, i;
...
{
    int last_nonb = 0, cur_nonb = 1, idx = 0, next_nonb;
...
static int scenecut( x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int real_scenecut, int num_frames, int i_max_search )
{
    ...
    {
        int origmaxp1 = p0 + 1, maxp1, curp1, curp0, i;
...
void x264_slicetype_analyse( x264_t *h, int intra_minigop )
{
    ...
    int keyframe, i, j, num_bframes, num_analysed_frames, reset_start;
