 ...
 ****************************************************************************/
static ALWAYS_INLINE int x264_cavlc_block_residual_escape( x264_t *h, int i_suffix_length, int level )
{
    bs_t *s = &h->out.bs;
...
static ALWAYS_INLINE void x264_cavlc_8x8_mvd( x264_t *h, int i )
{
...
static ALWAYS_INLINE void x264_cavlc_mb_header_b( x264_t *h, int i_mb_type, int chroma )
{
    ...
    else if( i_mb_type >= B_L0_L0 && i_mb_type <= B_BI_BI )
    {
        ...
        i_ref0_max = h->mb.pic.i_fref[0] - 1;
        i_ref1_max = h->mb.pic.i_fref[1] - 1;
