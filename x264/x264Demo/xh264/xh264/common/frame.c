...
static x264_frame_t *x264_frame_new( x264_t *h, int b_fdec )
{
   ...
   int i, j, p, luma_plane_size, chroma_padv;
    ...
    {
        luma_plane_size = align_plane_size( frame->i_stride[p] * (frame->i_lines[p] + 2*i_padv), disalign );
    ...
    if( i_csp == X264_CSP_NV12 || i_csp == X264_CSP_NV16 )
    {
        chroma_padv = i_padv >> (i_csp == X264_CSP_NV12);
...
{
    uint8_t *pix[3];
    int stride[3], b, v_shift, uv_swap;
    ...
    else
    {
        ...
        {
            uv_swap = i_csp == X264_CSP_YV12 || i_csp == X264_CSP_YV16;
...
void x264_frame_expand_border( x264_t *h, x264_frame_t *frame, int mb_y )
{
    ...
    int i, h_shift, v_shift, stride, width, height, padh, padv, starty; 
    pixel *pix;
    ...
    {
        h_shift = i && CHROMA_H_SHIFT;
        v_shift = i && CHROMA_V_SHIFT;
        stride = frame->i_stride[i];
        width = 16*h->mb.i_mb_width;
        height = (pad_bot ? 16*(h->mb.i_mb_height - mb_y) >> SLICE_MBAFF : 16) >> v_shift;
        padh = PADH;
        padv = PADV >> v_shift;
...
{
    int i, i_width, h_shift, v_shift, i_height, i_padx, i_pady, y;
    ...
    for( i = 0; i < frame->i_plane; i++ )
    {
        ...
        h_shift = i && CHROMA_H_SHIFT;
        v_shift = i && CHROMA_V_SHIFT;
        i_height = h->param.i_height >> v_shift;
        i_padx = (h->mb.i_mb_width * 16 - h->param.i_width);
        i_pady = (h->mb.i_mb_height * 16 - h->param.i_height) >> v_shift;
...
{
   int i, v_shift, stride, height, pady, y;
   pixel *fenc;
   
    for( i = 0; i < h->fenc->i_plane; i++ )
    {
        v_shift = i && CHROMA_V_SHIFT;
        stride = h->fenc->i_stride[i];
        height = h->param.i_height >> v_shift;
        pady = (h->mb.i_mb_height * 16 - h->param.i_height) >> v_shift;
        fenc = h->fenc->plane[i] + 16*mb_x;
