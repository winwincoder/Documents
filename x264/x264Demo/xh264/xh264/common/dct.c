...
static ALWAYS_INLINE void pixel_sub_wxh( dctcoef *diff, int i_size,
...
static void sub8x16_dct_dc( dctcoef dct[8], pixel *pix1, pixel *pix2 )
{
    ...
    a0 = sub4x4_dct_dc( &pix1[ 0*FENC_STRIDE+0], &pix2[ 0*FDEC_STRIDE+0] );
    a1 = sub4x4_dct_dc( &pix1[ 0*FENC_STRIDE+4], &pix2[ 0*FDEC_STRIDE+4] );
    a2 = sub4x4_dct_dc( &pix1[ 4*FENC_STRIDE+0], &pix2[ 4*FDEC_STRIDE+0] );
    a3 = sub4x4_dct_dc( &pix1[ 4*FENC_STRIDE+4], &pix2[ 4*FDEC_STRIDE+4] );
    a4 = sub4x4_dct_dc( &pix1[ 8*FENC_STRIDE+0], &pix2[ 8*FDEC_STRIDE+0] );
    a5 = sub4x4_dct_dc( &pix1[ 8*FENC_STRIDE+4], &pix2[ 8*FDEC_STRIDE+4] );
    a6 = sub4x4_dct_dc( &pix1[12*FENC_STRIDE+0], &pix2[12*FDEC_STRIDE+0] );
    a7 = sub4x4_dct_dc( &pix1[12*FENC_STRIDE+4], &pix2[12*FDEC_STRIDE+4] );
...
static void add4x4_idct( pixel *p_dst, dctcoef dct[16] )
{
    ...
    int i, s02, d02, s13, d13, y, x;
    for( i = 0; i < 4; i++ )
    {
        s02 =  dct[0*4+i]     +  dct[2*4+i];
        d02 =  dct[0*4+i]     -  dct[2*4+i];
        s13 =  dct[1*4+i]     + (dct[3*4+i]>>1);
        d13 = (dct[1*4+i]>>1) -  dct[3*4+i];
...
static void ALWAYS_INLINE add4x4_idct_dc( pixel *p_dst, dctcoef dc )
{
    int i;
    dc = (dc + 32) >> 6;
