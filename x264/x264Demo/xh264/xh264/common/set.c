...
static const uint16_t quant8_scale[6][6] =
{
 ...
 {
 	int i, size, start, j, q, i_list;
	int cat, dct8x8, dc_threshold,luma_threshold,chroma_threshold,max, thresh;
	double pos,bias;
	udctcoef *nr_offset;
	...
	int w, count;
	int w_1_q, w_2_q, t_1_q = 2, t_2_q = 3;
...
#define CQM_ALLOC( w, count )\
	...
	{\
	   ...
	   if( j < i )\
	    ...
	    for( j = 0; j < i; j++ )\
	    {\
	        if( deadzone[j] == deadzone[i] && !memcmp( h->pps->scaling_list[i+start], h->pps->scaling_list[j+start], size*sizeof(uint8_t) ) )\
	            break;\
	    }\
	    if( j < i )\
	    {\
