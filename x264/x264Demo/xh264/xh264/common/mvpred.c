...
static int x264_mb_predict_mv_direct16x16_temporal( x264_t *h )
{
    ...
    int yshift = 1;
     ...
     int cur_poc, col_parity, i_mb_4x4, i_mb_8x8;
     int max_i8,step,width,height,i8, x8,y8,ypart, i_part_8x8 , i_ref1_ref,i_ref;
    ...
    if( PARAM_INTERLACED && h->fref[1][0]->field[mb_xy] != MB_INTERLACED )
    {
        ...
        {
            cur_poc = h->fdec->i_poc + h->fdec->i_delta_poc[MB_INTERLACED&h->mb.i_mb_y&1];
            col_parity = abs(h->fref[1][0]->i_poc + h->fref[1][0]->i_delta_poc[0] - cur_poc)
    ...
    for(  i8 = 0; i8 < max_i8; i8 += step )
    {
         x8 = i8&1;
         y8 = i8>>1;
         ypart = (SLICE_MBAFF && h->fref[1][0]->field[mb_xy] != MB_INTERLACED) ?
        ...
        if( i_ref >= 0 )
        {
             dist_scale_factor = h->mb.dist_scale_factor[i_ref][0];
             mv_col = h->fref[1][0]->mv[0][i_mb_4x4 + 3*x8 + ypart * h->mb.i_b4_stride];
             mv_y = (mv_col[1]<<yshift)/2;
             l0x = ( dist_scale_factor * mv_col[0] + 128 ) >> 8;
             l0y = ( dist_scale_factor * mv_y + 128 ) >> 8;
...
{
    int8_t ref[2];
    int i_list, i_refa, i_refb, i_refc, i_ref, i_count;
    int16_t *mv_a, *mv_b, *mv_c;

     int mb_x,mb_y,mb_xy,cur_poc,col_parity,i_mb_4x4,i_mb_8x8;
     int type_col[2];
     int partition_col[2];
     ...
     int max_i8, step, width, height, i8,x8, y8, ypart, o8, o4, idx;
	
    ALIGNED_ARRAY_8( int16_t, mv,[2],[2] );
    for( i_list = 0; i_list < 2; i_list++ )
    {
        i_refa = h->mb.cache.ref[i_list][X264_SCAN8_0 - 1];
        mv_a  = h->mb.cache.mv[i_list][X264_SCAN8_0 - 1];
        i_refb = h->mb.cache.ref[i_list][X264_SCAN8_0 - 8];
        mv_b  = h->mb.cache.mv[i_list][X264_SCAN8_0 - 8];
        i_refc = h->mb.cache.ref[i_list][X264_SCAN8_0 - 8 + 4];
        mv_c  = h->mb.cache.mv[i_list][X264_SCAN8_0 - 8 + 4];
        if( i_refc == -2 )
        {
            i_refc = h->mb.cache.ref[i_list][X264_SCAN8_0 - 8 - 1];
            mv_c   = h->mb.cache.mv[i_list][X264_SCAN8_0 - 8 - 1];
    ...
    for( i8 = 0; i8 < max_i8; i8 += step )
    {
         x8 = i8&1;
         y8 = i8>>1;
         ypart = (b_interlaced && h->fref[1][0]->field[mb_xy] != MB_INTERLACED) ?
                    MB_INTERLACED ? y8*6 : 2*(h->mb.i_mb_y&1) + y8 :
                    3*y8;
         o8 = x8 + (ypart>>1) * h->mb.i_b8_stride;
         o4 = 3*x8 + ypart * h->mb.i_b4_stride;
...
#define SET_IMVP(xy) \
    if( xy >= 0 ) \
    { \
        shift = 1 + MB_INTERLACED - h->mb.field[xy]; \
        mvp = h->mb.mvr[i_list][i_ref<<1>>shift][xy]; \
        mvc[i][0] = mvp[0]; \
    ...
    {
        idx = i_list ? h->fref[1][0]->i_frame-h->fenc->i_frame-1
                         : h->fenc->i_frame-h->fref[0][0]->i_frame-1;
        if( idx <= h->param.i_bframe )
        {
            lowres_mv = h->fenc->lowres_mvs[i_list][idx];
...
#define SET_TMVP( dx, dy ) \
        { \
            mb_index = h->mb.i_mb_xy + dx + dy*h->mb.i_mb_stride; \
            scale = (curpoc - refpoc) * l0->inv_ref_poc[MB_INTERLACED&field]; \
