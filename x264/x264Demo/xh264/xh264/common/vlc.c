...
{
    int i_suffix, i_mask, abs_level, i_level_code, i_next;
    int size, bits, i, j, total, zeros, idx, run, len;
    int16_t level;
    uint32_t mask;
    vlc_large_t *vlc;
    for( i_suffix = 0; i_suffix < 7; i_suffix++ )
        ...
        {
             i_mask = level >> 15;
             abs_level = (level^i_mask)-i_mask;
             i_level_code = abs_level*2-i_mask-2;
             i_next = i_suffix;
             vlc = &x264_level_token[i_suffix][level+LEVEL_TABLE_SIZE/2];
    ...
    for( i = 1; i < (1<<16); i++ )
    {
        ...
        {
             idx = X264_MIN(zeros, 7) - 1;
             run = x264_clz( mask );
             len = run_before[idx][run].i_size;
            size += len;
            bits <<= len;
